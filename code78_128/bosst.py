...
#    X_train, y_train = X,y
    ...
    joblib.dump(regressor, './code78_128/model/xgbr.pkl')
#    print(regressor)
    submission(train_feature, regressor, df, './code78_128/submission/day000_submission_{}_01.csv'.format(lag),lag,log)
...
df = pd.read_csv('./code78_128/dataset/data_processed/day_infection_lag{}.csv'.format(lagging),delimiter=',', parse_dates =['data'])
...
lagging_feature=[x for x in df.columns.values.tolist() if x not in ['cityname', 'region_ID', 'data', 'infectnum',
                    'infectnum_WoW', 'infectnum_Sum', 'infectnum_mean', 'infectnum_max', 'infectnum_sum',
                    'infectnum_median','infectnum_std','infectnum_skew','migration','transfer','density','infectnum_min',
                    'infectnum_kurt','infectnum_quantile_25','infectnum_quantile_75','day_series','infectnum_WoW4','infectnum_Sum4','infectnum_WoW3','infectnum_Sum3']]
                    #'infectnum_mean', 'infectnum_max', 'infectnum_sum','infectnum_min',
...
#        best = train(df, params, best)
    ...
    if log==True:
        sub = pd.read_csv('./code78_128/submission/day000_submission_{}_01.csv'.format(lagging),header=None, names=['cityname', 'region_ID', 'data', 'infectnum'])
        ...
        sub.to_csv('./code78_128/submission/day000_submission_{}_01.csv'.format(lagging),header=False,index=False)
